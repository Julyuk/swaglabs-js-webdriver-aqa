...
describe('Checkout page', () => {
    ...
    async function addItemToCart(itemIndex) {
        expectedPrice = await (await inventoryPage.getItemPriceById(itemIndex)).getText();
        expectedDesc = await (await inventoryPage.getItemDescById(itemIndex)).getText();
        expectedName = await (await inventoryPage.getItemNameById(itemIndex)).getText();
        expectedQuantity = 1;
...
async function closeSideMenu(){
  ...
  checkoutTests.forEach(test => {
     ...
     it('Check "Cancel" button in the checkout page', async () => { 
      ...
      await cartPage.assertCartPageIsDisplayed()
    await verifyCartItemDetails(cartPage, expectedQuantity, expectedName, expectedDesc, expectedPrice);

});

     async function verifyCartItemDetails(page, expectedQuantity, expectedName, expectedDesc, expectedPrice) {
        assert.strictEqual((await page.cartItems.length), expectedQuantity)
        assert.strictEqual(await (await page.getItemNameById(0)).isDisplayed(), true)
    assert.strictEqual(await (await page.getItemNameById(0)).getText(), expectedName)
     assert.strictEqual(await (await page.getItemDescById(0)).isDisplayed(), true)
      assert.strictEqual(await (await page.getItemDescById(0)).getText(), expectedDesc)
       assert.strictEqual(await (await page.getItemPriceById(0)).isDisplayed(), true)
         assert.strictEqual(await(await page.getItemPriceById(0)).getText(), expectedPrice)
        assert.strictEqual(await(await page.getItemQuantityById(0)).isDisplayed(), true)
      assert.strictEqual(parseInt(await(await page.getItemQuantityById(0)).getText()), expectedQuantity)
     }
