...
class CheckoutPage extends Page {
    ...
    async assertInvalidCheckout(errorMsg){
        ...
        assert.strictEqual((await (await this.firstNameInput).getCSSProperty("border-bottom-color")).value, "rgba(226,35,26,1)")
        assert.strictEqual((await (await this.lastNameInput.getCSSProperty("border-bottom-color")).value, "rgba(226,35,26,1)")) 
        assert.strictEqual((await (await this.postalCodeInput.getCSSProperty("border-bottom-color")).value, "rgba(226,35,26,1)")) 
    }
}
