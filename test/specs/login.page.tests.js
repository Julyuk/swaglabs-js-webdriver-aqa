...
describe("Login page", () => {
  ...
  it("Login with invalid password", async () => {
    const invalidPassword =
      (await loginPage.generateRandomValue("number", 2, 5)) +
      (await loginPage.generateRandomValue("string", 3, 5));
    await loginAndAssert(
      await loginPage.getAcceptedUsernameById(0),
      invalidPassword,
      loginPage.errorUsernameAndPwdDoNotMAtch,
    );
  });
  ...
  it("Login with empty password field", async () => {
    await loginAndAssert(
      await loginPage.getAcceptedUsernameById(0),
      "",
      loginPage.pwdRequired,
    );
  });

  it("Login with locked out user", async () => {
    await loginAndAssert(
      await loginPage.getAcceptedUsernameById(1),
      await loginPage.getPasswordForAll(),
